...
import { Button, Paper, Table, Checkbox, Select, ButtonGroup } from '@mantine/core'
import { useDebouncedState } from '@mantine/hooks'
import { IconAd, IconAdjustmentsCheck, IconArcheryArrow, IconAssembly, IconChecks, IconCircleDashedLetterD, IconCubeSend, IconDetails, IconDetailsOff, IconEdit, IconFileExcel, IconFileExport, IconPlaylistAdd, IconUserDown, IconX, IconXboxX } from '@tabler/icons-react'
...
Itr_Assignment_List.title = "ITR Assignment"

export default function Itr_Assignment_List() {
...
const handleSelectRow = (id, checked) => {
  ...
  const hasViewPermission = usePermissions([17]);
  const hasUpdatePermission = usePermissions([40]);

  // PERMISSIONS EXPORT 
  const hasExportDataPermission = usePermissions([18])
  ...
  const columns = useMemo(() => [
...

    {
      ...
      cell: ({ row }) => (
        <Checkbox
          checked={selectedRowIds.includes(row.original.pcms_itr[0]?.id_itr)}
          onChange={(e) => handleSelectRow(row.original.pcms_itr[0]?.id_itr, e.currentTarget.checked)}
        />
      ),
      size: 40, 
    },
    ...
    //  assigned to PROCCESS
    {
      accessorFn: (row) => row.assigned_to_name?? "Not set",
      id: "production_assigned_to",
      header: "Assigned To",
      enableColumnFilter: true,
      enableSorting: true,
      cell: (info) => info.getValue(),
...
const getData = useCallback(async () => {
  ...
  try {
    const res = await axios.post(
      `${API_URL}/api/pcms_mc_template/assignment_list?${filterParams}&page=${pagination.pageIndex}&size=${pagination.pageSize}&sort=${sort}`,
       {
        filter: filterParams,
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (!q) return UsersOptions|| [];
    return (UsersOptions || []).filter(u => {
      const name = (u.full_name || u.fullname || u.username || "").toString().toLowerCase();
     const badge = (u.badge_no || u.badge || "").toString().toLowerCase();
...
const handleAssignITR = async () => {
  if (!selectedUser) {
    showAlert("Please select user", "warning");
    return;
  }

  if (selectedRowIds.length === 0) {
    showAlert("Please select at least one ITR item", "warning");
    return;
  }
  ...
  try {
    ...
    const payload = {
      id_itr : selectedRowIds,
      production_assigned_to: Number(selectedUser),
    }

    await axios.put(
      `${API_URL}/api/pcms_itr/assignment_user`,
      payload,
      {
        headers: { Authorization: `Bearer ${user.token}` },
      }
    ...
    showAlert("ITR successfully updated!", "success");
...
const handleExportExcel = async () => {
  try {
    ...
    const response = await axios.post(
      `${API_URL}/api/pcms_mc_template/export_excel`,
      payload,
      {
        headers: { Authorization: `Bearer ${user.token}` },
        responseType: "blob",
    ...
    a.href = url;
    a.download = "Pcms_ITR_Assignment.xlsx";
...
setColumnFilters(old => {
  ...
  return (
    ...
    {hasViewPermission ? (
        ...
        <div className="w-full">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-7">
              <label className="text-sm font-medium sm:w-32">Type Module :</label>
              <Select
      ...
      <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
        <div className="p-3 border-b flex justify-between bg-blue-800 text-white">
            <h1 className='text-center font-medium text-white p-1'> ITR ASSIGNMENT | Assignment ITR List </h1>
      ...
      {hasUpdatePermission && (
        ...
        >
        <label className="text-sm font-medium">Update FullName :</label>
