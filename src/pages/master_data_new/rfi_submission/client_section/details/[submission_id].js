...
import { IconArrowBack, IconAdjustmentsCheck, IconUpload, IconClearAll, IconClearFormatting, IconArrowRight, IconFilterPause, IconFolderPause, IconAdOff, IconSend2 } from '@tabler/icons-react'
...
DetailsByClient.title = "Details By Client"
export default function DetailsByClient() {
  ...
  // PEERMISSIONS
  const hasUpdatePermission = usePermissions([45]); 
  const hasUpdateReOffer = usePermissions([46]);
  const hasUpdatePostPone = usePermissions([47]);
  const hasUpdateApproveComment = usePermissions([48]); 
...
const fetchDetails = async () => {
  ...
  try {
    const res = await axios.get(
      `${API_URL}/api/pcms_itr/rfi-submission/${submission_id}`,
      { headers: { Authorization: `Bearer ${user.token}` } }
    );

    const list = (res.data.data || []).filter(
      row => Number(row.status_inspection) === 6
    );
 ...
 useEffect(() => {
  if (submission_id && !isAfterSubmit) {
    fetchDetails();
  }
}, [submission_id]); 
...
const handleSelectAll = (tag_number, type) => {
  ...
  const buildMcRel = () => {
    ...
    uniqueTags.forEach(tag_number => {
      ...
      if (row) {
        mcRel.push({
          itr_id: row.pcms_itr[0].id_itr ?? row.id,
  ...
  const handleSubmitRfi = async () => {
    ...
    const plWithoutRemarks = dedupedFormMC.filter(item => {
    const isPL = String(item.result).toUpperCase() === "PL";
    const remarksEmpty = !item.remarks || item.remarks.trim() === "";
    return isPL && remarksEmpty;
    });

    if (plWithoutRemarks.length > 0) {
      showAlert("Remarks must be filled when selecting PL.", "warning");
      setIsSubmitting(false);
      return;
    }
    ...
    if (!plByTag[tag]) plByTag[tag] = false;

    if (String(item.result).toUpperCase() === "PL") {
      plByTag[tag] = true;
    }
  });
  ...
  const hasPl = !!plByTag[r.tag_number];
  return {
    ...r,
    status_inspection: hasPl ? 7 : 8
  };
});

     const encryptedUser = encrypt(String(user.id));
    setIsSubmitting(true);

    const payload = {
      user_id: encryptedUser,
      created_by: encryptedUser,
    ...
    detail: {
      ...
      prev.map(row => {
        const match = mcRelWithStatus.find(m => 
          String(m.itr_id) === String(row.pcms_itr?.[0]?.id_itr ?? row.id)
        );
        ...
        return {
          ...row,
          status_inspection: match.status_inspection
        };
      })
    );

  // freeze agar tidak fetch ulang
    setIsAfterSubmit(true);

    } catch (err) {
      console.error("Failed to assign RFI:", err);
      showAlert("Failed to assign RFI. Please try again.", "error");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  // ------------ Handle Submit Re-Offer ----------------
   const handleReOffer = async () => {
    ...
    // set status_inspection = 10 for all selected mc_rel (postpone)
    const mcRelWithStatus = mcRel.map(r => ({
      ...r,
      status_inspection: 9,
    }));
    ...
    try {
      const res = await axios.post(`${API_URL}/api/pcms_checklist/reoffer_client`, payload, {
        headers: { Authorization: `Bearer ${user.token}` },
      });

      showAlert("RFI Data Successfully Submitted to Re-Offer!", "success");
  ...
  // ------------ Handle Submit PostPone ----------------
   ...
   const handlePostPone = async () => {
    ...
    try {
      const res = await axios.post(`${API_URL}/api/pcms_checklist/postpone_client`, payload, {
        headers: { Authorization: `Bearer ${user.token}` },
      });

      showAlert("RFI Data Successfully Submitted to Post-Pone!", "success");
  ...
  const hasAnyComment = dedupedFormMC.some(
    item => item.remarks && item.remarks.trim() !== ""
  );

  if (!hasAnyComment) {
    showAlert(
      "Approve with Comment requires at least one comment to be filled.",
      "warning"
    );
    setIsSubmitting(false);
    return;
  ...
  if (unselected.length > 0) {
    showAlert(
      "Please select OK / NA / PL for all checklist items before submitting.",
      "warning"
    );
    setIsSubmitting(false);
    return;
  ...
  try {
    const res = await axios.post(
      `${API_URL}/api/pcms_checklist/approve_client`,
      payload,
      { headers: { Authorization: `Bearer ${user.token}` } }
    );

    showAlert("Approve with Comment successfully submitted!", "success");

    window.dispatchEvent(
      new CustomEvent("rfi:updated", {
        detail: {
          updatedIds: (payload.mc_rel || []).map(m => m.itr_id),
          returned: res.data?.data ?? null,
        },
  ...
  setIsAfterSubmit(true);
    ...
    console.error("Approve with Comment failed:", err);
    showAlert("Failed to submit Approve with Comment.", "error");
  } finally {
    setIsSubmitting(false);
  }
};
...
const getStatusLabel = (status) => {
    switch (Number(status)) {
      case 6: return "Pending by Client";
      case 7: return "Reject by Client";
      case 8: return "Approve by Client";
      case 9: return "Re-Offer";
      case 10: return "Post-Pone";
      case 11: return "Approve With Comment";
      default: return "Status Unknown";
    }
  };

  // return tailwind classes for badge styling (soft background, rounded, spaced)
  const getStatusBadgeClass = (status) => {
    const s = Number(status);
    if (s === 6) return "bg-yellow-400 text-black border border-yellow-100";
    if (s === 7) return "bg-red-500 text-white border border-red-100";
    if (s === 8) return "bg-green-400 text-white border border-green-100";
    if (s === 9) return "bg-orange-500 text-white border border-orange-100";
    if (s === 10) return "bg-yellow-400 text-black border border-yellow-100";
    if (s === 11) return "bg-blue-400 text-black border border-blue-100";
        ...
        <div className="max-w-full mx-auto sm:px-6 lg:px-8 py-4">
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            <div className="p-4 border-b border-black flex justify-center bg-gray-200">
              ...
              PENDING APPROVAL DATA DETAILS – <span className="text-blue-800">{submission_id}</span>
              </h1>
            </div>
            ...
            <div className="px-4 py-2 text-left">
              <Button
                ...
                color="red"
                variant="outline"
                onClick={() => router.push('/master_data_new/rfi_submission/client_section/pending_client')}
          ...
          {openedPanels.length > 0 && !isAfterSubmit && (
          <div className="w-full mt-4 mb-8">
            ...
            <div className="flex items-center justify-between">
              ...
              {/* RIGHT - POSTPONE, RE-OFFER & Approve With Comment */}
              <div className="flex items-center gap-3">
                {hasUpdateReOffer & !isAfterSubmit && (
                <Button
                  color="orange"
                  size="sm"
                  onClick={handleReOffer}
                  leftSection={<IconAdOff/>}
                >
                  Submit Re-Offer
                </Button>
                )}

                {hasUpdatePostPone && !isAfterSubmit && (
                <Button
                  color="cyan"
                  size="sm"
                  onClick={handlePostPone}
                  leftSection={<IconFolderPause/>}
                >
                  Submit Post-Pone
                </Button>
                )}

                {hasUpdateApproveComment && !isAfterSubmit && (
                <Button
                  color="blue"
                  size="sm"
        ...
        {templateRows.map((row, idx) => (
          <Accordion.Item key={row.tag_number} value={row.tag_number}>
            ...
            <Accordion.Control>
              ...
              <div className="mt-2">
                <span className={`inline-flex items-center text-sm ${getStatusBadgeClass(row.status_inspection)} px-3 py-1 rounded-md`} >
                  {getStatusLabel(row.status_inspection)}
                </span>
              </div>
            </Accordion.Control>
            ...
            <Accordion.Panel>
              ...
              <div className="flex justify-between mb-4 mt-3">
                ...
                <Button.Group>
                  <Button color="green" onClick={() => handleSelectAll(row.tag_number, "OK")} variant="outline" >
                    OK All
                  </Button>
                  <Button color="blue" onClick={() => handleSelectAll(row.tag_number, "NA")} variant="outline">
                    NA All
                  </Button>
                  <Button color="orange" onClick={() => handleSelectAll(row.tag_number, "PL")} variant="outline">
                    PL All
                  </Button>
                </Button.Group>
              </div>
              ...
              {/* ======= TABLE ======== */}
              <table className="min-w-full border border-gray-400 mt-4 mb-4">
