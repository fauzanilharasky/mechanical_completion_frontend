...
export default function DetailsApproval() {
  ...
  // PERMISSIONS
  const hasUpdatePermissions = usePermissions([49])
...
const fetchDetails = async () => {
  ...
  try {
    ...
    const list = (res.data.data || []).filter(
      row => Number(row.status_inspection) === 8
    );
...
const handleSelectAll = (tag_number, type) => {
  ...
  const getStatusBadgeClass = (status) => {
    ...
    if (s === 8) return "bg-green-500 text-white border border-green-100";
        ...
        <div className="max-w-full mx-auto sm:px-6 lg:px-8 py-4">
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            ...
            <div className="px-4 py-2 text-left">
              <Button
                ...
                color="red"
                variant="outline"
                onClick={() => router.push('/master_data_new/rfi_submission/client_section/approve_client')}
          ...
          <Paper radius="sm" mt="lg" style={{ position: "relative" }} withBorder>
            <div className="p-4">
              ...
              {hasUpdatePermissions && openedPanels.length > 0 && (
