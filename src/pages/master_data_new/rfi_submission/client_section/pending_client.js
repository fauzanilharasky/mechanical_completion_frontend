...
import { IconAdjustmentsCheck, IconEdit, IconPlaylistAdd, IconX, IconUserDown, IconUpload, IconXboxX, IconCircleDashedLetterD, IconPdf, IconFileTypePdf, IconListCheck} from '@tabler/icons-react'
...
PendingClient.title = "Pending By Client"
export default function PendingClient() {
  ...
  // PERMISSIONS
    const hasViewPermission = usePermissions([29]);
  const hasDetailsDataPermission = usePermissions([30])
...
// ...existing code...
        ...
        cell: (info) => {
          ...
          return (
            <div className='align-middle items-center'>
             ...
             <Button
              onClick={() => router.push(`/master_data_new/rfi_submission/client_section/details/${value}`)}
  ...
  const getData = useCallback(async () => {
    ...
    searchQuery.status_inspection = 6;
    const filterParams =
    ...
    try {
        const res = await axios.post(
          `${API_URL}/api/pcms_mc_template/pending_client?${filterParams}&page=${pagination.pageIndex}&size=${pagination.pageSize}&sort=${sort}`,
          {
            filter: filterParams,
       ...
       const filteredData = rows.filter((item) => {
          ...
          return (
            ...
            Number(item.status_inspection ?? item.statusInspection ?? item.status ?? 0) === 6
          );
        });
  ...
  // EXPORT TO PDF 
    ...
    try {
      ...
      const res = await axios.get(
        `${API_URL}/api/pcms_itr/download_pdf/${submissionId}`,
        {
          responseType: "blob",
          headers: {
      ...
      <div className='py-6'>
        <div className="max-w-full mx-auto sm:px-6 lg:px-8 py-4">
                  ...
                  {/* Discipline */}
                   ...
                   <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-7">
                    ...
                    <input
                      ...
                      className="border rounded px-3 py-2 flex-1 text-sm"
                      value={columnFilters.find(f => f.id === "drawing_no")?.value || ""}
                      onChange={(e) => {
                        const value = e.target.value;

                        setColumnFilters(old => {
                          const otherFilters = old.filter(f => f.id !== "drawing_no");
                          return value
                            ? [...otherFilters, { id: "drawing_no", value }]
                            : otherFilters;
                        });
                      }}
                    />      
          ...
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            ...
            <h1 className='text-center font-medium px-3'> CLIENT SECTION | Pending Approval By Client </h1>
             </div>
             <Paper>
