...
RejectByClient.title = "Rejected By Client"
export default function RejectByClient() {
  ...
  // PERMISSIONS
  const hasDetailsDataPermission = usePermissions([32])
  const hasViewPermission = usePermissions([31]);
  ...
  const handleRejectRFI = async () => {
    ...
    const confirm = await Swal.fire({
      title: "Are you sure Client?",
      text: "Are you sure you want to continue with the rejected data that has been selected to be sent to QC?",
      icon: "question",
  ...
  const columns = useMemo(() => [
    ...
    {
      accessorFn: (row) => Number(row.status_inspection ?? row.statusInspection ?? 7),
      ...
      cell: (info) => {
        ...
        const statusOptions = (value === 7 || value === "7" || null)
          ? "Reject Submission"
          : "Pending Submission";
        ...
        return (
          <div className='align-middle items-center'>
            <Button.Group className='justify-center'>
              ...
              <Button
                onClick={() => router.push(`/master_data_new/rfi_submission/client_section/details/rejected_client/${value}`)}
  ...
  const getData = useCallback(async () => {
    ...
    searchQuery.status_inspection = 7;
    const filterParams =
    ...
    try {
      const res = await axios.post(
        `${API_URL}/api/pcms_mc_template/rejected_client?${filterParams}&page=${pagination.pageIndex}&size=${pagination.pageSize}&sort=${sort}`,
        {
          filter: filterParams,
      ...
      const filteredData = rows.filter((item) => {
        ...
        return (
          ...
          Number(item.status_inspection ?? item.statusInspection ?? 7) === 7
        );
      });
                ...
                <div className="w-full">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                   ...
                   <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-7">
                    ...
                    {/* submission No */}
                   <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-7">
                     <label className="text-sm font-medium sm:w-32">Report No :</label>
          ...
          <Paper radius="sm" mt="md" style={{ position: 'relative', }} withBorder>
            <Paper>
              <div className="p-4 border-b flex justify-start bg-blue-800 text-white">
                ...
                <h1 className='text-center font-medium px-3'> CLIENT SECTION | Reject Data List </h1>
              </div>
