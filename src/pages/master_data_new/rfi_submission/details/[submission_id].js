...
RfiSubmissionDetails.title = "Details Inspection"
export default function RfiSubmissionDetails() {
  ...
  const hasUpdatePermission = usePermissions([42]);
  ...
  const fetchDetails = async () => {
    ...
    try {
      ...
      const list = (res.data.data || []).filter(
        row => Number(row.status_inspection) === 1
      );
  ...
  const getStatusLabel = (status) => {
    switch (Number(status)) {
      case 1: return "Pending Submission by SPV";
      case 2: return "Reject By Spv";
      case 3: return "Pending Approval By QC";
      case 4: return "Postponed";
      case 5: return "Ready for Transmittal";
      case 6: return "Transmitted";
  ...
  const getStatusBadgeClass = (status) => {
    ...
    if (s === 1) return "bg-yellow-400 text-black border border-white";
    if (s === 2) return "bg-red-500 text-white border border-white-100";
    if (s === 3) return "bg-yellow-200 text-black border border-yellow-100";
 ...
 useEffect(() => {
  ...
  // --------- MC_REL proccess (REVISION SUBMIT ITR_ID[0]=> ARRAY) --------
  ...
  return {
    ...r,
    status_inspection: hasPl ? 2 : 3
  };
});
    
    const encryptedCreatedBy = encrypt(String(user.id));
    setIsSubmitting(true);
    
    const payload = {
      user_id: user.id,
      created_by: encryptedCreatedBy,
    ...
    try {
      const res = await axios.post(`${API_URL}/api/pcms_checklist/assignment_rfi`, payload, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
      
      
      showAlert("RFI successfully Submit!", "success");
     ...
     // update status_inspection secara local
        ...
        <div className="max-w-full mx-auto sm:px-6 lg:px-8 py-4">
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            <div className="p-4 border-b border-black flex justify-center bg-gray-200">
              ...
              <h1 className="font-bold text-xl justify-center">
                Inspection RFI Details – <span className="text-blue-800">{submission_id}</span>
              </h1>
            </div>
            ...
            <div className="px-4 py-2 text-left">
              <Button
                ...
                color="red"
                variant="outline"
                onClick={() => router.push('/master_data_new/rfi_submission/inspection_rfi')}
          ...
          <Paper radius="sm" mt="lg" style={{ position: "relative" }} withBorder>
            <div className="p-4">
              ...
              {hasUpdatePermission && openedPanels.length > 0 && !isAfterSubmit && (
                <div div className="w-full flex flex-col items-start mt-4 mb-8">
                  <Button
                    color="green"
                    size="sm"
                  ...
                  <Accordion.Item key={row.tag_number} value={row.tag_number}>
                    ...
                    <Accordion.Panel>
                      ...
                      <div className="flex justify-between mb-4 mt-4">
                        ...
                        <Button.Group>
                          <Button color="green" onClick={() => handleSelectAll(row.tag_number, "OK")}>
                            OK All
                          </Button>
                          <Button color="blue" onClick={() => handleSelectAll(row.tag_number, "NA")}>
                            NA All
                          </Button>
                          <Button color="orange" onClick={() => handleSelectAll(row.tag_number, "PL")}>
                            PL All
                          </Button>
                        </Button.Group>
                      </div>
