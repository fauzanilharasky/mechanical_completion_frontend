...
DetailsBySpv.title = "Details By Supervisor"
export default function DetailsBySpv() {
  ...
  const fetchDetails = async () => {
    ...
    try {
      const res = await axios.get(
        `${API_URL}/api/pcms_itr/details/${submission_id}`,
        { headers: { Authorization: `Bearer ${user.token}` } }
      );

      const list = (res.data.data || []).filter(
        row => Number(row.status_inspection) === 2
      );
 ...
 const getStatusLabel = (status) => {
    switch (Number(status)) {
      case 1: return "Pending approval by Spv";
  ...
  const getStatusBadgeClass = (status) => {
    ...
    if (s === 2) return "bg-red-500 text-white border border-red-100";
    if (s === 3) return "bg-green-50 text-green-800 border border-green-100";
    if (s === 5) return "bg-blue-50 text-blue-800 border border-blue-100";
    if (s === 6) return "bg-indigo-50 text-indigo-800 border border-indigo-100";
        ...
        <div className="max-w-full mx-auto sm:px-6 lg:px-8 py-4">
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            <div className="p-4 border-b border-black flex justify-center bg-gray-200">
              ...
              <h1 className="font-bold text-xl justify-center">
                DETAILS DATA REJECTED BY CLIENT – <span className="text-blue-800">{submission_id}</span>
              </h1>
            </div>
            ...
            <div className="px-4 py-2 text-left">
              <Button
                ...
                color="red"
                variant="outline"
                onClick={() => router.push('/master_data_new/rfi_submission/reject_rfi')}
          ...
          <Paper radius="sm" mt="lg" style={{ position: "relative" }} withBorder>
            ...
            <div div className="w-full flex flex-col items-start mt-4 mb-8">
              <span className="mb-1 text-gray-700 text-sm"> click the button here to upload</span>
              <Button
                color="green"
                size="sm"
                            ...
                            <tr key={item.item_no}>
                              ...
                              <td className="border px-3 py-2">
                                <Textarea
                                  ...
                                  placeholder="Type remarks..." disabled
                                  autosize
                                  minRows={1}
                                  maxRows={4}
                                />
