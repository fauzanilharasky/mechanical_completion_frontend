...
import { IconAdjustmentsCheck, IconEdit, IconPlaylistAdd, IconX, IconUserDown, IconUpload, IconXboxX, IconCircleDashedLetterD, IconPdf, IconFileTypePdf, IconSettingsSearch, IconSend, IconCubeSend} from '@tabler/icons-react'
...
RfiInspection.title = "Pending Approval Spv"
export default function RfiInspection() {
  ...
  // PERMISSIONS DATA
   const hasViewPermission = usePermissions([21]);
  const hasDetailsDataPermission = usePermissions([23])
...
// ...existing code...
        ...
        cell: (info) => {
          ...
          return (
            <div className='align-middle items-center'>
             ...
             <Button
              onClick={() => router.push(`/master_data_new/rfi_submission/details/${value}`)}
  ...
  const getData = useCallback(async () => {
    ...
    searchQuery.status_inspection = 1;
    const filterParams =
    ...
    try {
      if (!user?.token) return;
        const res = await axios.post(
          `${API_URL}/api/pcms_mc_template/inspection_rfi?${filterParams}&page=${pagination.pageIndex}&size=${pagination.pageSize}&sort=${sort}`,
          {
            filter: filterParams,
      ...
      ...item,
      status_inspection: Number(item.status_inspection),
      assignment_status: Number(item.assignment_status),
    }));
        
        setData(normalizedData);
        setTotalPages(res.data.total_pages?? 0 );
        setTotalEntries(res.data?.totalElements || normalizedData.length);
      } catch (error) {
  ...
  // EXPORT TO PDF 
    ...
    try {
      ...
      const res = await axios.get(
        `${API_URL}/api/pcms_itr/generate_pdf/${submissionId}`,
        {
          responseType: "blob",
          headers: {
    ...
    <AuthLayout sidebarList={rfiSubmissionList}>
      ...
      <div className='py-6'>
        <div className="max-w-full mx-auto sm:px-6 lg:px-8 py-4">
          ...
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
             <div className="p-4 border-b text-white flex justify-start bg-blue-700">
               <IconCubeSend size={26} />
            <h1 className='text-center font-medium px-3'> SUPERVISOR SECTION | Inspection RFI List </h1>
             </div>
             <Paper>

            <div className="p-4 overflow-x-auto">
              
              <Datatables table={table} totalPages={totalPages} pagination={pagination}  info={{ totalElements: totalEntries }} />
