...
import { IconAdjustmentsCheck, IconEdit, IconPlaylistAdd, IconX, IconUserDown, IconUpload, IconXboxX, IconCircleDashedLetterB, IconFileTypePdf, IconPlayerEject, IconDeviceProjector, IconMapDown, IconDatabaseMinus } from '@tabler/icons-react'
...
RejectSubmission.title = "Rejected RFI"
export default function RejectSubmission() {
  ...
  const {encrypt: encryptData} = useEncrypt();


  // PERMISSIONS DATA
   const hasViewPermission = usePermissions([20]);
  const hasDetailsDataPermission = usePermissions([22])
  ...
  const handleRejectRFI = async () => {
    ...
    const confirm = await Swal.fire({
      title: "Are you sure?",
      text: "Do you want to continue Submit and submit the reject selected RFI data?",
      icon: "question",
    ...
    const encryptedUser = encryptData(String(user.id));
    setSubmitting(true);

    try {
      const payload = {
        id_template: selectedRowIds,
        requestor: encryptedUser,
        area_v2: selectedArea,
        location_v2: selectedLocation,
        date_request: new Date(),
        submission_id: true,
        spv_inspection_by: user.id,
  ...
  const columns = useMemo(() => [
    ...
    {
      accessorFn: (row) => Number(row.status_inspection ?? row.statusInspection ?? 2),
      ...
      cell: (info) => {
        ...
        const statusOptions = (value === 2 || value === "2" || null)
          ? "Reject Submission"
          : "Pending Submission";
        ...
        return (
          <div className='align-middle items-center'>
            <Button.Group className='justify-center'>
              ...
              <Button
                 onClick={() => router.push(`/master_data_new/rfi_submission/details/reject_spv/${value}`)}
  ...
  const getData = useCallback(async () => {
    ...
    searchQuery.status_inspection = 2;
    const filterParams =
    ...
    try {
      ...
      const res = await axios.post(
        `${API_URL}/api/pcms_mc_template/rejected_list?${filterParams}&page=${pagination.pageIndex}&size=${pagination.pageSize}&sort=${sort}`,
        {
          filter: filterParams,
      ...
      // Hanya tampilkan baris yang memiliki submission_id (tidak null/undefined/empty)
      const filteredData = rows.filter((item) => {
        const hasSubmission = item.submission_id !== null &&
          item.submission_id !== undefined &&
          String(item.submission_id).trim() !== "";
        return (
          hasSubmission &&
          Number(item.assignment_status) === 1 &&
          Number(item.status_inspection ?? item.statusInspection ?? 2) === 2
        );
      });

      setData(filteredData);

      // set totalPages dari server (fallback jika tidak tersedia)
      const totalFromServer = res.data.total_pages ?? Math.ceil((res.data.total_rows ?? rows.length) / (pagination.pageSize || 10));
      setTotalPages(totalFromServer);
          ...
          <Paper radius="sm" mt="md" style={{ position: 'relative', }} withBorder>
            <Paper>
              <div className="p-4 border-b flex justify-start bg-blue-800 text-white">
                ...
                <h1 className='text-center font-medium px-3'> SUPERVISOR SECTION | Reject Data List </h1>
              </div>
