...
SubmissionQcDetails.title = "Details Inspection QC"
export default function SubmissionQcDetails() {
  ...
  // PERMISSIONS
  const hasUpdatePermission = usePermissions([43]);
...
const fetchDetails = async () => {
  ...
  try {
    ...
    const list = (res.data.data || []).filter(
      row => Number(row.status_inspection) === 3
    );
 ...
 const getStatusLabel = (status) => {
    switch (Number(status)) {
      ...
      case 4: return "Reject By QC";
      case 5: return "Pending Transmittal By QC";
  ...
  const getStatusBadgeClass = (status) => {
    ...
    if (s === 3) return "bg-yellow-400 text-black border border-green-100";
    if (s === 4) return "bg-red-500 text-white border border-white-100";
    if (s === 5) return "bg-blue-400 text-white border border-blue-100";
...
const handleSelectAll = (tag_number, type) => {
  ...
  const handleSubmitRfi = async () => {
    ...
    const confirm = await Swal.fire({
      title: 'Are you sure?',
      text: "Do you want to submit the selected inspection results",
      icon: "question",
  ...
  return {
    ...r,
    status_inspection: hasPl ? 4 : 5
  };
});

    const encryptedCreateBy = encrypt(String(user.id));
    setIsSubmitting(true);

    const payload = {
      user_id: user.id,
      created_by: encryptedCreateBy,
    ...
    try {
      const res = await axios.post(`${API_URL}/api/pcms_checklist/assignment_qc`, payload, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
          ...
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            ...
            <div className="px-4 py-2 text-left">
              <Button
                ...
                color="red"
                variant="outline"
                onClick={() => router.push('/master_data_new/rfi_submission/submission_qc/pending_sub_qc')}
