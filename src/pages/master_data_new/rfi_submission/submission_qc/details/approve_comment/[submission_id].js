...
ApproveComment.title = "Details Approve With Comment"
...
const fetchDetails = async () => {
  ...
  try {
    ...
    const list = (res.data.data || []).filter(
      row => Number(row.status_inspection) === 11
    );
...
const handleSelectAll = (tag_number, type) => {
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      const res = await axios.post(`${API_URL}/api/pcms_checklist/submit_comment`, payload, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
  ...
  const getStatusBadgeClass = (status) => {
    ...
    if (s === 11) return "bg-blue-400 text-white border border-white";
          ...
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            <div className="p-4 border-b border-black flex justify-center bg-gray-200">
              ...
              <h1 className="font-bold text-xl justify-center">
                DETAILS DATA APPROVE WITH COMMENT BY QC – <span className="text-blue-800">{submission_id}</span>
              </h1>
            </div>
