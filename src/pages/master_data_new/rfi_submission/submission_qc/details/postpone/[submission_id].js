...
const fetchDetails = async () => {
  ...
  try {
    ...
    const list = (res.data.data || []).filter(
      row => Number(row.status_inspection) === 10
    );
...
const handleSelectAll = (tag_number, type) => {
  ...
  const handleSubmitPostPone = async () => {
    ...
    try {
      const res = await axios.post(`${API_URL}/api/pcms_checklist/submit_postpone`, payload, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
  ...
  const getStatusBadgeClass = (status) => {
    ...
    if (s === 10) return "bg-yellow-400 text-black border border-white";
          ...
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            <div className="p-4 border-b border-black flex justify-center bg-gray-200">
              ...
              <h1 className="font-bold text-xl justify-center">
                DETAILS DATA POST-PONE BY QC – <span className="text-blue-800">{submission_id}</span>
              </h1>
            </div>
