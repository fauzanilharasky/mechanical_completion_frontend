...
QcSubmissionDetails.title = "Details Inspection"
export default function QcSubmissionDetails() {
...
const fetchDetails = async () => {
  ...
  try {
    ...
    const list = (res.data.data || []).filter(
      row => Number(row.status_inspection) === 4
    );
...
const handleSelectAll = (tag_number, type) => {
  ...
  const handleSubmitRfi = async () => {
    ...
    const mcRelWithStatus = mcRel.map(r => {
      ...
      return { ...r, status_inspection: hasPl ? 2 : 3 };
    });
  ...
  const getStatusBadgeClass = (status) => {
    ...
    if (s === 4) return "bg-red-500 text-white border border-red-100";
          ...
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            <div className="p-4 border-b border-black flex justify-center bg-gray-200">
              ...
              <h1 className="font-bold text-xl justify-center">
                DETAILS DATA REJECTED BY QC – <span className="text-blue-800">{submission_id}</span>
              </h1>
            </div>
            ...
            <div className="px-4 py-2 text-left">
              <Button
                ...
                color="red"
                variant="outline"
                onClick={() => router.push('/master_data_new/rfi_submission/submission_qc/reject_qc')}
