...
TransmittalDetails.title = "Transmittal By QC"
export default function TransmittalDetails() {
...
const fetchDetails = async () => {
  ...
  try {
    ...
    const list = (res.data.data || []).filter(
      row => Number(row.status_inspection) === 5
    );
...
const hasChanges = (tag_number) => {
  const currentResults = selectedResults?.[tag_number] || {};
  const initialResults = initialResultsState?.[tag_number] || {};

  const currentRemarks = remarksValues?.[tag_number] || {};
  const initialRemarks = initialRemarksState?.[tag_number] || {};

  // cek perubahan result
  const resultChanged = Object.keys(currentResults).some(
    key => currentResults[key] !== initialResults[key]
  );

  // cek perubahan remarks
  const remarksChanged = Object.keys(currentRemarks).some(
    key => currentRemarks[key] !== initialRemarks[key]
  );

  return resultChanged || remarksChanged;
};
...
const handleSelectAll = (tag_number, type) => {
  ...
  const handleSubmitRfi = async () => {
    ...
    const mcRelWithStatus = mcRel.map(r => {
      ...
      return { ...r, status_inspection: hasPl ? 4 : 5 };
    });
          ...
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            ...
            <div className="px-4 py-2 text-left">
              <Button
                ...
                color="red"
                variant="outline"
                onClick={() => router.push('/master_data_new/rfi_submission/submission_qc/transmittal_by_qc')}
