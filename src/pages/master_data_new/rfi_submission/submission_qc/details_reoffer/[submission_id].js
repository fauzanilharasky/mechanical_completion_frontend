...
ReTransmittal.title = "Details Re-Transmittal"
export default function ReTransmittal() {
...
const fetchDetails = async () => {
  ...
  try {
    ...
    const list = (res.data.data || []).filter(
      row => Number(row.status_inspection) === 9
    );
...
const handleSelectAll = (tag_number, type) => {
  ...
  const handleSubmitReOffer = async () => {
    ...
    dedupedFormMC.forEach(item => {
      const itr = item.itr_id ?? itrMap?.[item.tag_number] ?? null;
    ...
    const mcRelWithStatus = mcRel.map(r => {
      ...
      return { ...r, status_inspection: hasPl ? 6 : 7 };
    });
    ...
    try {
      const res = await axios.post(`${API_URL}/api/pcms_checklist/submit_reoffer`, payload, {
        headers: { Authorization: `Bearer ${user.token}` },
      });


      showAlert("Checklist successfully Submit!", "success");
  ...
  const getStatusBadgeClass = (status) => {
    ...
    if (s === 9) return "bg-orange-500 text-white border border-white";
          ...
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            <div className="p-4 border-b border-black flex justify-center bg-gray-200">
              ...
              <h1 className="font-bold text-xl justify-center">
                DETAILS DATA RE-OFFER BY QC – <span className="text-blue-800">{submission_id}</span>
              </h1>
            </div>
            ...
            <div className="px-4 py-2 text-left">
              <Button
                ...
                color="red"
                variant="outline"
                onClick={() => router.push('/master_data_new/rfi_submission/submission_qc/approval_review')}
          ...
          <Accordion.Item key={row.tag_number} value={row.tag_number}>
            ...
            <Accordion.Panel>
              ...
              <div className="flex justify-between mb-4 mt-3">
                ...
                <Button.Group>
                  <Button color="green" variant="outline" onClick={() => handleSelectAll(row.tag_number, "OK")}>
                    OK All
                  </Button>
                  <Button color="blue" variant="outline" onClick={() => handleSelectAll(row.tag_number, "NA")}>
                    NA All
                  </Button>
                  <Button color="orange" variant="outline" onClick={() => handleSelectAll(row.tag_number, "PL")}>
                    PL All
                  </Button>
                </Button.Group>
              </div>
