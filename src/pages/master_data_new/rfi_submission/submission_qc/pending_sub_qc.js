...
import { IconAdjustmentsCheck, IconEdit, IconPlaylistAdd, IconX, IconUserDown, IconUpload, IconXboxX, IconCircleDashedLetterD, IconPdf, IconFileTypePdf, IconListCheck, IconFileExport} from '@tabler/icons-react'
...
PendingQc.title = "Pending Approval QC"
export default function PendingQc() {
  ...
  // PERMISSIONS DATA
   const hasViewPermission = usePermissions([24]);
   const hasDetailsDataPermission = usePermissions([25])
...
// ...existing code...
        ...
        cell: (info) => {
          ...
          return (
            <div className='align-middle items-center'>
             ...
             <Button
              onClick={() => router.push(`/master_data_new/rfi_submission/submission_qc/details/${value}`)}
  ...
  const getData = useCallback(async () => {
    ...
    searchQuery.status_inspection = 3;
    const filterParams =
    ...
    try {
        const res = await axios.post(
          `${API_URL}/api/pcms_mc_template/pending_Qc?${filterParams}&page=${pagination.pageIndex}&size=${pagination.pageSize}&sort=${sort}`,
          {
            filter: filterParams,
       ...
       const filteredData = rows.filter((item) => {
          ...
          return (
            ...
            Number(item.status_inspection ?? item.statusInspection ?? item.status ?? 3) === 3
          );
        });
...
useEffect(() => {
  ...
  try {
    ...
    const res = await axios.get(
      `${API_URL}/api/pcms_itr/export_pdf/${submissionId}`,
      {
        responseType: "blob",
        headers: {
      ...
      <div className='py-6'>
        <div className="max-w-full mx-auto sm:px-6 lg:px-8 py-4">
          ...
          <Paper radius="sm" mt="md" style={{ position: 'relative' }} withBorder>
            ...
            <h1 className='text-center font-medium px-3'> QC SECTION | Pending Approval By QC </h1>
             </div>
             <Paper>
