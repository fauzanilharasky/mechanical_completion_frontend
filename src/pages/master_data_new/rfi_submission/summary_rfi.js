                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  {/* Discipline */}
                   ...
                   {/* Status */}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-7">
                     <label className="text-sm font-medium sm:w-32"> Status Submission :</label>
                     <Select
                        className="flex-1"
                        placeholder="Select Status Inspection"
                        data={Object.entries(STATUS_INSPECTION_MAP).map(([k, v]) => ({
                            value: k.toString(), 
                            label: v.label,
                        }))}
                        onChange={(val) => {
                            setColumnFilters(old => [
                            ...old.filter(f => f.id !== "status_inspection"),
                            ...(val !== null
                                ? [{ id: "status_inspection", value: Number(val) }]
                                : [])
                            ]);
                        }}
                        clearable
